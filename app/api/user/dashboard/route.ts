...
export async function GET(request: Request) {
  try {
    ...
    const [subscriptionResult, todayResult, recentResult, devicesResult] = await Promise.all([
      auth.supabase
        .from("subscriptions")
    ...
    const error = subscriptionResult.error || todayResult.error || recentResult.error || devicesResult.error
    if (error) {
