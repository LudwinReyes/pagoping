    ...
    <div className={`min-h-screen bg-background ${isDashboardBlocked ? "relative" : ""}`}>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-background/95 backdrop-blur-sm">
          <Card className="max-w-md mx-4">
            ...
            <CardContent className="space-y-3">
              <Button
                className="w-full bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-700 hover:to-indigo-700 text-white font-semibold shadow-md"
                onClick={() => setPaymentModalOpen(true)}
              >
                <Sparkles className="mr-2 h-4 w-4" />
                Pagar y Activar con Yape
              </Button>
      ...
      <main className="container mx-auto p-3.5 sm:p-5 md:p-6 space-y-5 sm:space-y-6 pb-28 md:pb-12">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          ...
          {/* Vencimiento / Plan Upgrade Button */}
          <div className="flex flex-wrap items-center gap-2">
            {subscription?.ends_at && !isExpired && tier !== "free" ? (
              <button
                onClick={() => setPaymentModalOpen(true)}
                className="flex items-center justify-between sm:justify-start gap-2.5 px-3.5 py-2 rounded-2xl sm:rounded-full bg-white dark:bg-gray-800 border border-purple-200 dark:border-purple-800 shadow-sm hover:shadow-md transition-all duration-200 group w-full sm:w-auto text-left"
              >
                <div className="flex items-center gap-2.5">
                  ...
                  <div className="text-left">
                    <p className="text-[10px] sm:text-xs font-semibold text-purple-600 dark:text-purple-400 uppercase tracking-wide">Vence el</p>
                ...
                <Badge variant="outline" className="text-purple-600 border-purple-300 bg-purple-50 dark:bg-purple-950/50 text-[11px] ml-2 group-hover:bg-purple-600 group-hover:text-white transition-colors">
                  Renovar
                </Badge>
              </button>
            ) : (
              <Button
                onClick={() => setPaymentModalOpen(true)}
                className="bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-700 hover:to-indigo-700 text-white font-medium text-xs sm:text-sm shadow-sm rounded-full"
              >
                <Sparkles className="w-4 h-4 mr-1.5" />
                Adquirir / Activar Plan
              </Button>
            )}
          </div>
        </div>
      ...
      {/* Modal de Pago de Suscripción con Yape Pasarela */}
